...
export function CoursesScreen() {
      ...
      try {
        ...
        setCategories([
          ...
          ...categorySnapshot.docs.map((docSnap) => ({
            id: docSnap.id,
            name: docSnap.data()?.name || 'Category',
          })),
        ]);
        ...
        const coursesWithCategory = await Promise.all(
          courseSnapshot.docs.map(async (courseDoc) => {
            ...
            if (teacherId) {
              try {
                ...
                if (teacherSnap.exists()) {
                  ...
                  teacherName = teacherData?.name || teacherData?.fullName || 'Instructor';
                }
              } catch (innerErr) {
            ...
            return {
              ...
              totalLessons:
                data.totalLessons || data.total_lessons || data.lessonsCount || data.lessons,
              totalModules:
                data.totalModules || data.total_modules || data.modulesCount || data.modules,
            };
          })
        );
  ...
  const filteredCourses = courses.filter((course) => {
    ...
    const matchesSearch = term
      ? (course.title || course.name || '').toString().toLowerCase().includes(term) ||
        (course.description || '').toString().toLowerCase().includes(term) ||
        (course.instructor || '').toString().toLowerCase().includes(term) ||
        (course.categoryName || '').toString().toLowerCase().includes(term)
      : true;
    ...
    <View style={styles.container}>
      <View style={styles.hero}>
        ...
        <Text style={styles.heroSubtitle}>
          Choose from our comprehensive selection of recorded courses and interactive live sessions
        </Text>
      </View>
      ...
      <View style={styles.segmented}>
        <Pressable
          style={[styles.segment, activeTab === 'recorded' && styles.segmentActive]}
          onPress={() => setActiveTab('recorded')}>
          <Text style={[styles.segmentText, activeTab === 'recorded' && styles.segmentTextActive]}>
            Recorded Courses ({segmentedCounts.recorded})
          </Text>
        </Pressable>
        <Pressable
          style={[styles.segment, activeTab === 'interactive' && styles.segmentActive]}
          onPress={() => setActiveTab('interactive')}>
          <Text
            style={[styles.segmentText, activeTab === 'interactive' && styles.segmentTextActive]}>
...
// ستايلات زي ما عندك بدون تغيير أسماء
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: Colors.background, padding: 16 },
  hero: { marginBottom: 16, alignItems: 'center', gap: 8 },
  heroTitle: {
  ...
  filterRow: { flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 12, zIndex: 5 },
  searchBox: {
    flex: 1,
    flexDirection: 'row',
    alignItems: 'center',
  ...
  dropdownContainer: { width: 180, position: 'relative' },
  dropdownTrigger: {
    flexDirection: 'row',
    alignItems: 'center',
  ...
  dropdownText: { fontFamily: Fonts.poppins, fontSize: FontSizes.base, color: Colors.foreground },
  dropdownMenu: {
    position: 'absolute',
    top: 50,
    right: 0,
  ...
  dropdownItemActive: { backgroundColor: Colors.muted },
  dropdownItemText: {
    fontFamily: Fonts.poppins,
    fontSize: FontSizes.base,
    color: Colors.foreground,
  },
  dropdownItemTextActive: { color: Colors.primary, fontWeight: FontWeights.medium },
  segmented: {
    flexDirection: 'row',
  ...
  cover: { width: '100%', height: 220, backgroundColor: Colors.muted },
  coverPlaceholder: { alignItems: 'center', justifyContent: 'center', gap: 6 },
  placeholderText: { fontFamily: Fonts.poppins, color: Colors.mutedForeground },
  badge: {
    position: 'absolute',
    top: 14,
    right: 14,
  ...
  chipText: { fontFamily: Fonts.poppins, color: Colors.foreground, fontWeight: FontWeights.medium },
  description: {
  ...
  instructor: { fontFamily: Fonts.cairo, fontSize: FontSizes.h4, color: Colors.primary },
  metaRow: { flexDirection: 'row', gap: 16, marginTop: 4 },
  metaItem: { flexDirection: 'row', alignItems: 'center', gap: 6 },
  metaText: { fontFamily: Fonts.cairo, fontSize: FontSizes.base, color: Colors.mutedForeground },
  actionsRow: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    alignItems: 'center',
